...
export default ({ config, db }) => {
	...
	api.all('/login', async (req, res) => {
		...
		try {
			...
			let userinfo = await XueshengModel.findOne({ where: { xueshengxuehao: req.query.username, mima: req.query.password } })
			if (userinfo === null) {
			...
			const token = jwt.sign(
				{
					...
					username: userinfo.dataValues.xueshengxuehao,
					role: '学生',
                    loginUserColumn: 'xueshengxuehao',
                    tableName: 'xuesheng'
				},
				config.jwtSecret,
				{
			...
			userinfo.dataValues.loginUserColumn = 'xueshengxuehao'
	...
	api.all('/resetPass', async (req, res) => {
		...
		try {
			...
			await XueshengModel.update({
				...
				where: {
				  xueshengxuehao: req.query.username
				}
			})
	...
	api.get('/page', async (req, res) => {
		...
		try {
			...
			let where = {}
			var xueshengxuehao = req.query.xueshengxuehao
			if (xueshengxuehao) {

				if (xueshengxuehao.indexOf('%') != -1) {
					where.xueshengxuehao = {
						[Op.like]: xueshengxuehao
					}
				} else {
					where.xueshengxuehao = {
						[Op.eq]: xueshengxuehao
					}
				}
			}
