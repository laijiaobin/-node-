      ...
      <el-col :span="24">  
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.gerenjianli" label="个人简历" prop="gerenjianli">
            <el-button type="text" size="small" @click="download($base.url+ruleForm.gerenjianli)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      <el-col :span="12">
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `rutuanshenqing/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.rutuanshenqingCrossAddOrUpdateFlag = false;
						 this.parent.search();
