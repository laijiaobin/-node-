    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="新生攻略" prop="xinshenggonglve">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.xinshenggonglve" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.xinshenggonglve" label="新生攻略" prop="xinshenggonglve">
                    <span v-html="ruleForm.xinshenggonglve"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.faburiqi = true;
          ...
          if(o=='xinshenggonglve'){
            this.ruleForm.xinshenggonglve = obj[o];
	    this.ro.xinshenggonglve = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.xinshenggonglve = this.ruleForm.xinshenggonglve.replace(reg,'../../../nodejsi2pjk/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `xinshenggonglve/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.xinshenggonglveCrossAddOrUpdateFlag = false;
						 this.parent.search();
