...
export default {
  ...
  methods: {
	...
	this.ruleForm.shetuanjieshao = this.ruleForm.shetuanjieshao.replace(reg,'../../../nodejsi2pjk/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shetuanjianjie/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shetuanjianjieCrossAddOrUpdateFlag = false;
						 this.parent.search();
